            ...
            <div className="bg-slate-900 border border-slate-700 rounded-xl p-4 shadow-lg">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-slate-300">
                    <div className="flex justify-between items-center bg-slate-800/50 p-2 rounded">
                        ...
                        <span className="font-bold text-white">{state.subFase1?.acero?.toFixed(1) || 0} t</span>
                    </div>
                    <div className="flex justify-between items-center bg-slate-800/50 p-2 rounded">
                        <span>Masa Negra (A Buffer):</span> 
                        <span className="font-bold text-yellow-400">{state.subFase1?.masaNegra?.toFixed(1) || 0} t</span>
                    </div>
                    <div className="flex justify-between items-center bg-slate-800/50 p-2 rounded">
                        <span>Cobre / Aluminio:</span> 
                        <span className="font-bold text-white">{state.subFase1?.cobre?.toFixed(1) || 0} t</span>
                    </div>
                    <div className="flex justify-between items-center bg-slate-800/50 p-2 rounded">
                        <span>Agua Purificada:</span> 
                        <span className="font-bold text-blue-400">{state.subFase1?.agua?.toFixed(1) || 0} t</span>
                    </div>
                    <div className="flex justify-between items-center bg-slate-800/50 p-2 rounded">
                        <span>Plásticos/Polímeros:</span> 
                        <span className="font-bold text-white">{state.subFase1?.plasticos?.toFixed(1) || 0} t</span>
                    </div>
                    <div className="flex justify-between items-center bg-slate-800/50 p-2 rounded border-l-2 border-red-500/50">
                        ...
                        <span className="font-bold text-red-400">{state.subFase1?.residuos?.toFixed(1) || 0} t</span>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-xl p-4 shadow-lg">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-slate-300">
                    <div className="flex justify-between items-center bg-slate-800/50 p-2 rounded">
                        ...
                        <span className="font-bold text-white">{state.subFase2?.NiSO4?.toFixed(1) || 0} t</span>
                    </div>
                    <div className="flex justify-between items-center bg-slate-800/50 p-2 rounded">
                        <span>Sulfato Cobalto (CoSO₄):</span> 
                        <span className="font-bold text-white">{state.subFase2?.CoSO4?.toFixed(1) || 0} t</span>
                    </div>
                    <div className="flex justify-between items-center bg-slate-800/50 p-2 rounded">
                        <span>Sulfato Manganeso (MnSO₄):</span> 
                        <span className="font-bold text-white">{state.subFase2?.MnSO4?.toFixed(1) || 0} t</span>
                    </div>
                    <div className="flex justify-between items-center bg-slate-800/50 p-2 rounded">
                        <span>Hidróxido Litio (LiOH):</span> 
                        <span className="font-bold text-white">{state.subFase2?.LiOH?.toFixed(1) || 0} t</span>
                    </div>
                    <div className="flex justify-between items-center bg-slate-800/50 p-2 rounded">
                        <span>Agua Recuperada:</span> 
                        <span className="font-bold text-blue-400">{state.subFase2?.agua?.toFixed(1) || 0} t</span>
                    </div>
                    <div className="flex justify-between items-center bg-slate-800/50 p-2 rounded border-l-2 border-red-500/50">
                        <span>Sólidos Residuales:</span> 
                        <span className="font-bold text-red-400">{state.subFase2?.residuos?.toFixed(1) || 0} t</span>
                    </div>
                </div>
            </div>

            {/* ALERTA DE DERIVACIÓN */}
            <div className="mt-auto bg-red-950/20 border border-red-900/50 p-4 rounded-xl flex justify-between items-center">
